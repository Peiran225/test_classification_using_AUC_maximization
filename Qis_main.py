...
class AUCTrainer(Trainer):
	def __init__(self, *args, p=1 / (1 + 0.2), manually_schedule_lr=False, lambda_reg=0, learning_rate_1=1e-3, learning_rate_2=1e-3, **kwargs):
		...
		self.manually_schedule_lr = manually_schedule_lr
	...
	def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # update w value 
		self.w = ( torch.mean((self.p * y_pred * (0 == labels).float() - (1 - self.p) * y_pred * (1==labels).float()))) / (self.p * (1 - self.p))
				...
				if (
					...
					if optimizer_was_run:
						...
						if not isinstance(self.lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
							if self.manually_schedule_lr==False:
							    self.lr_scheduler.step()
							else:
								if 0.45 <= self.state.epoch < 0.455: #1000 in total foe 32 batch
									print(self.state.epoch)
									for _ in range(0,110): #100 end up with 0.67 auc in on epoch
										self.lr_scheduler.step()
...
def main(args):
   ...
   # Train 
	...
	trainer = AUCTrainer(
		...
		manually_schedule_lr = args.manually_schedule_lr
	)
	# L2_norm_square = 0
	# L2_norm_sq_grad = 0
	# for _, p in model.named_parameters():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--manually_schedule_lr", default=False)

	
	args = parser.parse_args()
	args.warmup_ratio = 0.1
    args.manually_schedule_lr = True
	...
	print(f'lr1 {args.learning_rate_1} lr2 {args.learning_rate_2} weight decay {args.weight_decay} warmup_ratio {args.warmup_ratio} \
	   num_train_epochs {args.num_train_epochs} lr_scheduler_type {args.lr_scheduler_type} manually_schedule_lr {args.manually_schedule_lr}' )
